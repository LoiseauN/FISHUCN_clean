...
all_geo_res$taux <- all_geo_res$taux/max(abs(all_geo_res$taux))
...
text(x=3200,y=0.05,"positive",col="darkgray",cex=1.3) 
text(x=3200,y=-.05,"negative",col="darkgray",cex=1.3)
...
for (i in 1:nrow(all_geo_res)){
  ...
  if(all_geo_res$taux[i]< -0.5) {coloo=wes_palette(name="Zissou1")[1]}
  if(all_geo_res$taux[i]< 0 & all_geo_res$taux[i]> -0.5) {coloo=wes_palette(name="Zissou1")[2]}
  if(all_geo_res$taux[i]< 0.5 & all_geo_res$taux[i]> 0) {coloo=wes_palette(name="Zissou1")[3]}
  if(all_geo_res$taux[i] > 0.5) {coloo=wes_palette(name="Zissou1")[5]}
  
  segments(x0=all_geo_res$richness[i],
           x1=all_geo_res$richness[i],
           y0=0,
           y1=all_geo_res$taux[i], col = coloo,lwd=2)
...
dens <- density(all_geo_res$richness)
